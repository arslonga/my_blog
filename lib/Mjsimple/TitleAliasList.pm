...
foreach my $item( @levels ){
    my @title_alias_curr = $c->menu->find( $item, 
                                           ['distinct(title_alias)'], 
                                           {
                                            'children' => [{'!=', 'yes'}, undef], 
                                            'in_menu' => 'yes'
                                           } 
...
foreach my $curr_level( @levels ){
    my @title_alias_curr = $c->menu->find( $curr_level, 
                                           ['distinct(title_alias)'], 
                                           {'children' => [{'!=', 'yes'}, undef]} 
                                         )->flat;
    foreach(@title_alias_curr){
        my $title = $c->menu->find( $curr_level, 
                                    ['title'], 
                                    {'title_alias' => $_} 
                                  )->list;
        $_ = $_.'|'.$title;
