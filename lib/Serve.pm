...
foreach my $curr_parent_dir(@parent_dir){
    ...
    $hashref->{level} = $curr_level;
        if( $hash_dir_subs{$hashref->{'title_alias'}} ){
            ...
            if($curr_level ne 'level0'){
                $self->menu->save( $curr_level, 
                                   {list_enable => 'no'}, 
                                   {title_alias => $title_alias}
                                 );
                my $url_in_search = $self->menu->find( $search_table, 
                                                       ['url'], 
                                                       {table_name => $title_alias} 
                                                     )->list;
                eval{ $self->menu->delete( $search_table, {'url' => $url_in_search}); };
            }
        }
...
if( ($data->{'list_enable'} eq 'yes' && $data->{level} ne 'level0') || 
($data->{'list_enable'} eq 'no' && $data->{level} eq 'level0')){
$menu_form.=<<HTML;
description:<br>
...
if( ($data->{'children'} && $data->{'children'} ne 'yes') && 
    $data->{'level'} ne 'level0' ){
