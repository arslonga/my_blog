...
<div class="panel">
    <div class="acrd">
        <a class="collapsed" data-toggle="collapse" data-parent="#accordion" href="#$year">
          $year
        </a>
    </div>
    <div id="$year" class="panel-collapse collapse">
      <div id="archive_month_block">
      <ul>
ARCH_MENU
    ...
    #----------------------------------        
        ...
        $common_menu.="<li id=\"li_archive_month\"><a href=\"/archive/$year/$month\" class=\"archive_month_label\">".$c->lang_config->{'num_month'}->{$month}->{$language}.'</a> <span class="cnt-arch">['.$cnt.']</span>'."</li>\n";
    }
...
my($id) = $c->menu->find( $table_archive_menu, ['id'], {'url' => 'common'} )->list;
if(!$id){
    $c->menu->create( $table_archive_menu, {'url' => 'common', menu => $common_menu} );
}else{
    $c->menu->save( $table_archive_menu, {menu => $common_menu}, {'url' => 'common'} );
}
    ...
    foreach my $curr_url_compl( @archive_url ){
        ...
        my($url_exist) = $c->menu->find( $table_archive_menu, ['id'], {'url' => $curr_url} )->list;
        if(!$url_exist){
            $c->menu->create( $table_archive_menu, {'url' => $curr_url, menu => $common_menu} );
        }else{
            $c->menu->save( $table_archive_menu, {menu => $common_menu}, {'url' => $curr_url} );
        }
...
my(%exist, %date_Data_hash, %date_Data_hash_mod, @levels, @title_alias, $ref_pagination_attr, 
   $tw_og, $limit_str, $message, @other_data, @other_month, $current_page, $total_pages, $i_begin, $i_end);
my $url = $self->req->url;
$url =~ s/\?page\=.*$//;
my $url_mod = $url;
...
my($description, $keywords) = $self->menu->find( 'level0', ['description', 'keywords'] )->list;
($ref_pagination_attr, $current_page, $limit_str) = $self->routine_model->pagination_attr( $self, 'pagination_attr_file' );
@levels = @{$self->top_config->{'levels'}};

foreach( @levels ){
    my @title_alias_curr = $self->menu->find( $_, ['distinct(title_alias)'], {'in_menu' => 'yes'} )->flat;
...
foreach my $curr_table( @title_alias ){    
    ...
    while( my $data = $result->hash ){
        my($rubric) = $self->menu->find( $table_rubric, ['rubric'], {'id' => "$data->{'rubric_id'}"} )->list if($data->{'rubric_id'} != 0);
        $data->{'rubric'} = uc $rubric if $rubric;
        $data->{'children'} = $self->menu->find( $data->{'level'}, ['children'], {'id' => "$data->{'level_id'}"} )->list;
...
if (!(keys %date_Data_hash) || $url_components[-1] eq '0'){
    goto EN;
}
...
my($archive_menu) = $self->menu->find( $table_archive_menu, ['menu'], {'url' => $url} )->list;
